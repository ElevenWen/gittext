...
​	get  请求的数据都在地址栏中，数据容易泄露

​	传输的数据有长度限制：4kb(不同浏览器长度不同)

​	默认都是使用  get  请求

post请求：

​	有请求体。而且我们的数据都是通过请求体提交的数据，不会显示到地址栏中

表单数据    get请求中如果不添加name属性则提交不了
...
```javascript
//send方法才是真的的发起网络请求。 参数：请求的时候向服务区传递的参数。 如果是get请求，直接传入null即可。如果是 post 请求，就是具体请求的数据。
xhr.send(null);
```
...
json对象   -->js对象       用一对{   }括起来的就是一个json对象     存储的键值对：如果键和值是字符串的话，则必须加引号（数字类型可以不用加引号，布尔类型也会被当做字符串，所有也需要加引号）

json数组   -->js数组         用一对[  ]括起来的就是一个json对象
